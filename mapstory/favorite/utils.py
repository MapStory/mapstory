...
def get_favorite_info(user, content_object):
    ...
    result["add_url"] = reverse("add_favorite_{}".format(
        url_content_type), args=[content_object.pk])

    existing_favorite = models.Favorite.objects.favorite_for_user_and_content_object(
        user, content_object)

    if existing_favorite:
        result["has_favorite"] = "true"
        result["delete_url"] = reverse(
            "delete_favorite", args=[existing_favorite.pk])
    else:
