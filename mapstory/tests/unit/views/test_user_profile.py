...
from mapstory.views import profile_edit
...
class ProfileDetailViewTest(MapStoryTestMixin):
    ...
    def test_profile_edit_page_responses(self):
        ...
        self.assertEqual(other_url, u'/storyteller/edit/%s/' % other_user.username)
    ...
    def test_profile_edit_doesnt_exist(self):
        # Should not find this user
        self.userclient.login_as_non_admin(
            username=self.test_username,
            password=self.test_password
        ...
        response = self.userclient.get(
            reverse('profile_edit', kwargs={'username': 'nonexistent'}),
            follow=True
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_profile_edit_no_profile_exception(self):
        ...
        # TODO: Discover why we are getting a forbidden http error
        # self.assertEqual(response.status_code, 200)
    ...
    def test_profile_delete_anonymous_user_delete(self):
        ...
        response = self.client.get(
            reverse('profile_delete', kwargs={'username': 'nonexistentuser'}),
            follow=True
        )
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'account/login.html')
    ...
    def test_profile_delete_post(self):
        ...
        response = self.client.get(
            reverse('profile_detail', kwargs={'slug': self.test_username}),
            follow=True
        )
        self.assertEqual(response.status_code, 200)
