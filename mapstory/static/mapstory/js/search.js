...
(function() {
  ...
  module.load_active_list = function($http, $rootScope, $location, api, endpoint, filter, value) {
    ...
    $http.get(endpoint, {
      ...
      if (api === 'categories') {
        ...
        data.objects = _.where(data.objects, {
          'description': ''
        });
      }
  ...
  module.controller('geonode_search_controller', function($injector, $scope, $location, $http, $q, Configs, Django) {
    ...
    function query_api(data) {
      ...
      return $http.get($scope.api_endpoint, {
        params: data || {}
      }).success(function(data) {
    ...
    function possible_profiles(token) {
      ...
      for (var i = 0; i < num_spaces; i++) {
        ...
        // query api w/query
        promises[i] = $http.get('/api/profiles/', {
            params: query
          })
          .success(function(data) {
            var results = data.objects;
      ...
      promises.push($http.get('/api/profiles/', {
          params: query
        })
        .success(function(data) {
          var results = data.objects;
