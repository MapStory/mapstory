describe('Data Service', () => {
  ...
  describe('Region Service:', () => {
	  ...
	  it('should should be an object with get() and _modify() methods', () => {
	  ...
	  describe('results for autocomplte via ._modify()', () => {
	    let mockResult; let apiCount; let randomEntry;
	    ...
	    it('should return an object with "all" and "byCodes" properties', () => {
	    ...
	    it('each should have the same number of entries as the mockAPI response', () => {
	      const allCount = mockResult.all.length;
	      const byCodesCount = Object.keys(mockResult.byCodes).length;

	      expect(allCount).to.equal(apiCount);
	      expect(byCodesCount).to.equal(apiCount);
	    });
	 
	 		describe('array of all country results', () => {
	      it('each should still have properties "code" and "name" that are strings', () => {
	        expect(randomEntry.code).to.be.a('string');
	        expect(randomEntry.name).to.be.a('string');
	      });

	  		it('each should now have a property "_lower" with lowercased name & code for autocomplete', () => {
	        const name = randomEntry.name;
	        const code = randomEntry.code;
	        const _lower = randomEntry._lower;
	        ...
	        expect(code.toLowerCase()).to.equal(_lower[1]);
	 		...
	 		describe('index of countries by codes', () => {
	 			it('should return an identical region object given a regions code', () => {
	        const byCodes = mockResult.byCodes;
	        const code = randomEntry.code;
	  ...
	  describe('[dev hack] mock JSON of /api/regions response', () => {
	    it('should be the same length as geonode APIs expected list of regions', () => {
	      const regions = mockRegions.data.objects;

	      expect(regions).to.have.lengthOf(259, 'the number of regions has changed');
	    });

	    it('regions should have properties "code" and "name" that are strings', () => {
	      const code = mockRegions.data.objects[111].code;
	      const name = mockRegions.data.objects[222].name;
