...
.controller('collection_controller', function($http, $scope) {
  $scope.query = function(group_id) {
    $http.get('/api/collections/').then(function(response) {
      ...
      $scope.facebook = data.group.org.social_facebook.split('/')[1];
      $scope.twitter = data.group.org.social_twitter.split('/')[1];
      $scope.tasks = data.group.org.tasks;
      $scope.interests = data.group.keywords;
      $scope.summary = data.group.description;
      $scope.city = data.group.org.city;
      $scope.country = data.group.org.country;
