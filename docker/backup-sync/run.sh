...
cd /backups
aws s3 cp --no-progress --recursive $BAK_DIR s3://mapstory-prod-backup/daily/$TODAY

# Weekly backup (on Sunday)
if [ $(date -u +%w) -eq 0 ]
then
    echo "Pushing weekly to S3"
    aws s3 cp --no-progress --recursive $BAK_DIR s3://mapstory-prod-backup/weekly/$TODAY
fi
